-- 050_screener_book_universe.sql — book holdings are screenable even when untagged.
--
-- v_screener's universe was ticker_tags only, so a book holding that Hedgeye never
-- tagged (sector ETFs like XLV, XLP, TLT, ...) could never surface in a "my book"
-- screen. Widen the base universe to ticker_tags UNION the held book underlyings
-- (latest snapshot, cash & zero-qty already excluded by the `held` CTE). Untagged
-- book names appear as rows with NULL tags (gics_sector/subsector/bucket) but with
-- MFR range/trend joined; MFR-untracked ones carry has_range=false and fall to the
-- caller's DARK section. Non-"my book" screens still filter on hedgeye_bucket_0629,
-- so NULL-bucket ETFs stay out of the single-name universe — behavior there is
-- unchanged from 047. Everything else (columns, COALESCE trend, momentum, vol) is
-- identical to 047.
