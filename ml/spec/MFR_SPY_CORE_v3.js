// MFR_SPY_CORE_v3 — state + RATE-OF-CHANGE feature set for SPY daily ML.
// 6 fetches (runtime limit): #MFR_SPY_LO, #MFR_SPY_HI, #MFR_SPY_BULLDIST, #MFR_VIX_LO, #MFR_VIX_HI, HYG.
...
describe_indicator('MFR_SPY_CORE_v3', 'lower', { decimals: 3 });
const RES = 'D';
...
const hi_d3 = for_every(pct(HI, lag(HI, 3)), (v) => (isNum(v) ? clamp(v, -0.1, 0.1) : NaN));   // HH (+) vs LH (−)
const lo_d3 = for_every(pct(LO, lag(LO, 3)), (v) => (isNum(v) ? clamp(v, -0.1, 0.1) : NaN));   // HL (+) vs LL (−)
